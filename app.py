...
def create_monthly_sheet(month_name, opening_balance):
...
def create_month():
    ...
    try:
        ...
        month_display = get_current_month_display()
        
        # Check if month already exists
        months_data = load_months_data()
        if current_month in months_data:
            return jsonify({'success': False, 'message': 'This month already exists!'})
        
        # Create Excel file
        filename = create_monthly_sheet(current_month, opening_balance)
...
def get_stats():
    ...
    # Calculate total records across all months
    total_records = 0
    total_in_payment = 0
    total_out_payment = 0
    total_references = 0
